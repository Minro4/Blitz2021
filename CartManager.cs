...
namespace Blitz2020
{
    class CartManager
    {
        ...
        public List<UnitAction> updateCart(GameMessage message, MinerManager minersManager, MapManager mapManager)
        {
            ...
            var availableMiners = miners.Where(miner => { return travelingChariots.Find(
        ...
        private void cleanCarts(List<Unit> karts, GameMessage message)
        {
            ...
            //Add new chariots
            var newKarts = message.getMyCrew().get(Unit.UnitType.CART).Where((kart) => { return chariots.Find((chariot) => chariot.id == kart.id) != null; })
                .ToList();
